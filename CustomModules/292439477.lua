...
local function getcustomassetfunc(path)
	if not isfile(path) then
		...
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/NBDMatr1x/BedwarsMatr1Hub/main/"..path:gsub("Matr1xHub/assets", "assets"),
			Method = "GET"
		})
...
local function isPlayerTargetable(plr, target, friend)
    return plr ~= lplr and plr and (friend and friendCheck(plr) == nil or (not friend)) and shared.Matr1xteamcheck(plr)
end
...
teleportfunc = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started then
		if shared.Matr1xkickedfrom then
			queueteleport('shared.Matr1xkickedfrom = "'..shared.Matr1xkickedfrom..'"')
		end
	end
end)
		...
		if obj.Name == "ErrorMessage" then
			obj:GetPropertyChangedSignal("Text"):Connect(function()
				if obj.Text:find("votekicked") and AutoRejoin["Enabled"] then
					shared.Matr1xkickedfrom = shared.Matr1xkickedfrom and shared.Matr1xkickedfrom..game.JobId..'/' or game.JobId..'/'
					...
					for i,v in pairs(List) do
						if i ~= game.JobId then
							...
							if MaxCheck and SlowCheck and GoodPlayers and findjobid(i, shared.Matr1xkickedfrom) then
								ChosenServer = i
								break
							end
						end
			...
			if callback then
				BindToRenderStep("TriggerBot", 1, function()
					for i,plr in pairs(players:GetChildren()) do
						if lplr ~= plr and shared.Matr1xteamcheck(plr) and type(phantomforces["Ray"].raycastwhitelist) == "table" then 
							local char = phantomforces["GetCharacter"](plr)
							if char then
...
BindToRenderStep("LegitRender", 500, function()
	...
	for i,plr in pairs(players:GetChildren()) do
		if lplr ~= plr and shared.Matr1xteamcheck(plr) then 
					...
					for i,plr in pairs(players:GetChildren()) do
						...
						if ESPDrawing["Enabled"] then 
							if ESPMethod["Value"] == "2D" then
								...
								if plr ~= lplr and (ESPTeammates["Enabled"] or shared.Matr1xteamcheck(plr)) then
									local char = phantomforces["GetCharacter"](plr)
    								if char then
										local allowed = false
				...
				BindToRenderStep("Tracers", 500, function()
					for i,plr in pairs(players:GetChildren()) do
							...
							if TracersDrawing["Enabled"] then
								...
								if plr ~= lplr and (TracersTeammates["Enabled"] or shared.Matr1xteamcheck(plr)) then
									local char = phantomforces["GetCharacter"](plr)
				...
				BindToRenderStep("NameTags", 500, function()
					for i,plr in pairs(players:GetChildren()) do
						...
						if NameTagsDrawing["Enabled"] then
							...
							if plr ~= lplr and (NameTagsTeammates["Enabled"] or shared.Matr1xteamcheck(plr)) then
								local char = phantomforces["GetCharacter"](plr)
								if char then

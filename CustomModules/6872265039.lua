...
local whitelisted = {
	...
	chattags = {
		["a"] = {
			NameColor = {r = 254, g = 0, b = 0},
			Tags = {
				{
					TagColor = {r = 254, g = 0, b = 0},
					TagText = "okay"
				}
			}
		}
...
teleportfunc = lplr.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started then
		if shared.Matr1xoverlay then
			queueteleport('shared.Matr1xoverlay = "'..shared.Matr1xoverlay..'"')
		end
		...
		if callback then
			spawn(function()
				repeat
					...
					firstqueue = false
					if shared.Matr1xteammembers and bedwars["ClientStoreHandler"]:getState().Party then
						repeat task.wait() until #bedwars["ClientStoreHandler"]:getState().Party.members >= shared.Matr1xteammembers or JoinQueue["Enabled"] == false
					end
...
runcode(function()
	local tpstring = shared.Matr1xoverlay or nil
...
if shared.nobolineupdate then
	...
	spawn(function()
		...
		local window = kavo.CreateLib("Noboline v1.6.2"..(shared.Matr1xPrivate and " - PRIVATE" or ""), "Ocean")

...
local clients = {
	ChatStrings1 = {
		["KVOP25KYFPPP4"] = "Matr1x Hub",
		["IO12GP56P4LGR"] = "future",
		["RQYBPTYNURYZC"] = "rektsky"
	},
	ChatStrings2 = {
		["Matr1x Hub"] = "KVOP25KYFPPP4",
...
runcode(function()
    getfunctions = function()
		...
		if not shared.Matr1xbypassed then
...
local function getNametagString(plr)
	...
	if bedwars["CheckPlayerType"](plr) == "Matr1x Hub PRIVATE" then
		nametag = '<font color="rgb(127, 0, 255)">[Matr1x Hub PRIVATE] '..(plr.Name)..'</font>'
	end
	...
	if (bedwars["CheckPlayerType"](plr) ~= "DEFAULT" or whitelisted.chattags[bedwars["HashFunction"](plr.Name..plr.UserId)]) then
		...
		if lplr ~= plr and bedwars["CheckPlayerType"](lplr) == "DEFAULT" then
			task.spawn(function()
				...
				task.wait(4)
				repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..clients.ChatStrings2.Matr1x, "All")
				task.spawn(function()
					local connection
					for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
						if newbubble:IsA("TextLabel") and newbubble.Text:find(clients.ChatStrings2.Matr1x) then
...
runcode(function()
	...
	local killauranoMatr1x = {["Enabled"] = false}
	...
	local function newAttackEntity(plr, firstplayercodedone, attackedplayers)
		...
		if killauranoMatr1x["Enabled"] and clients.ClientUsers[plr.Player.Name] then
			return nil
		end
  ...
  --[[  killauratargets = Killaura.CreateSlider({
	...
	if bedwars["CheckPlayerType"](lplr) ~= "DEFAULT" then
		killauranoMatr1x = Killaura.CreateToggle({
			["Name"] = "No Matr1x Hub",
			["Function"] = function() end,
			["HoverText"] = "no hit Matr1x Hub user"
		})
	end
end)
...
lagbackevent.Event:connect(function(plr)
	if AutoToxic["Enabled"] then
		if AutoToxicLagback["Enabled"] then
			...
			local msg = custommsg or "Imagine lagbacking L "..(plr.DisplayName or plr.Name).." | Matr1x Hub on top"
    ...
    if myTeam and myTeam.id == winstuff.winningTeamId and victorysaid == false then
		...
		if AutoToxic["Enabled"] then
			...
			if AutoToxicWin["Enabled"] then
				repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(#AutoToxicPhrases["ObjectList"] > 0 and AutoToxicPhrases["ObjectList"][math.random(1, #AutoToxicPhrases["ObjectList"])] or "EZ L TRASH KIDS | Matr1x Hub on top", "All")
			end
		end
    end
end)
...
local function findplayers(arg)
	...
	if arg == "private" and continuechecking and bedwars["CheckPlayerType"](lplr) == "Matr1x Hub PRIVATE" then table.insert(temp, lplr) continuechecking = false end
...
runcode(function()
	...
	chatconnection = repstorage.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:connect(function(tab, channel)
		...
		if plr and bedwars["CheckPlayerType"](lplr) ~= "DEFAULT" and tab.MessageType == "Whisper" and client ~= nil and alreadysaidlist[plr.Name] == nil then
			...
			createwarning("Matr1x Hub", plr.Name.." is using "..client.."!", 60)
		...
		if (AutoToxicTeam["Enabled"] == false and lplr:GetAttribute("Team") ~= plr:GetAttribute("Team") or AutoToxicTeam["Enabled"]) and (#AutoToxicPhrases5["ObjectList"] > 0 and toxicfindstr(tab["Message"], AutoToxicPhrases5["ObjectList"]) or #AutoToxicPhrases5["ObjectList"] == 0 and (tab["Message"]:lower():find("hack") or tab["Message"]:lower():find("exploit") or tab["Message"]:lower():find("cheat"))) and plr ~= lplr and table.find(ignoredplayers, plr.UserId) == nil and AutoToxic["Enabled"] and AutoToxicRespond["Enabled"] then
			...
			local msg = custommsg or "I dont care about the fact that I'm hacking, I care about how you died in a block game L "..(plr.DisplayName or plr.Name).." | Matr1x Hub on top"
			repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(msg, "All")
		...
		connectionstodisconnect[#connectionstodisconnect + 1] = p13:Connect(function(p14)
			...
			if AutoToxic["Enabled"] then
				if AutoToxicBedDestroyed["Enabled"] and p14.brokenBedTeam.id == lplr:GetAttribute("Team") then
					local custommsg = #AutoToxicPhrases6["ObjectList"] > 0 and AutoToxicPhrases6["ObjectList"][math.random(1, #AutoToxicPhrases6["ObjectList"])] or "How dare you break my bed >:( <name> | Matr1x Hub on top"
					if custommsg then
				...
				elseif AutoToxicBedBreak["Enabled"] and p14.player.UserId == lplr.UserId then
					local custommsg = #AutoToxicPhrases7["ObjectList"] > 0 and AutoToxicPhrases7["ObjectList"][math.random(1, #AutoToxicPhrases7["ObjectList"])] or "nice bed <teamname> | Matr1x Hub on top"
					if custommsg then
			...
			if p7.fromEntity == lplr.Character and p7.entityInstance ~= lplr.Character then 
				...
				if AutoToxic["Enabled"] then 
					...
					if plr and plr:GetAttribute("Spectator") and AutoToxicFinalKill["Enabled"] then
						local custommsg = #AutoToxicPhrases2["ObjectList"] > 0 and AutoToxicPhrases2["ObjectList"][math.random(1, #AutoToxicPhrases2["ObjectList"])] or "L <name> | Matr1x Hub on top"
						if custommsg == lastsaid then
							custommsg = #AutoToxicPhrases2["ObjectList"] > 0 and AutoToxicPhrases2["ObjectList"][math.random(1, #AutoToxicPhrases2["ObjectList"])] or "L <name> | Matr1x Hub on top"
						else
							lastsaid = custommsg
						end
						if custommsg then
			...
			if leavesaid == false and p7.entityInstance == lplr.Character then
				...
				if bedwars["GamePlayerUtil"].getGamePlayer(lplr):isSpectator() then
					...
					if plr and AutoToxic["Enabled"] and AutoToxicDeath["Enabled"] then
						local custommsg = #AutoToxicPhrases3["ObjectList"] > 0 and AutoToxicPhrases3["ObjectList"][math.random(1, #AutoToxicPhrases3["ObjectList"])] or "My gaming chair expired midfight, thats why you won <name> | Matr1x Hub on top"
						if custommsg then
							custommsg = custommsg:gsub("<name>", (plr.DisplayName or plr.Name))
						end
						repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(custommsg, "All")
...
runcode(function()
	...
	local function finishcframe(cframe)
		return shared.Matr1xOverrideAnticheatBypassCFrame and shared.Matr1xOverrideAnticheatBypassCFrame(cframe) or cframe
	end
	...
	local function disablestuff()
		...
		shared.Matr1xRealCharacter = {
			Humanoid = entity.character.Humanoid,
			Head = entity.character.Head,
			HumanoidRootPart = oldcloneroot
		}
		if shared.Matr1xOverrideAnticheatBypassPre then 
			shared.Matr1xOverrideAnticheatBypassPre(lplr.Character)
		end
		repeat
			task.wait()
			if entity.isAlive then
				...
				if oldroot then
					...
					if cloneroot then
						if oldroot.Parent ~= nil and ((networkownerfunc and (not networkownerfunc(oldroot)) or networkownerfunc == nil and gethiddenproperty(oldroot, "NetworkOwnershipRule") == Enum.NetworkOwnership.Manual)) then
							...
							if not shared.Matr1xOverrideAnticheatBypass then
	...
	local function anticheatbypassenable()
		task.spawn(function()
			...
			spawncoro = true
			allowspeed = false
			shared.Matr1xRealCharacter = nil
			repeat task.wait() until entity.isAlive
			task.wait(0.4)
...
runcode(function()
	tpstring = shared.Matr1xoverlay or nil
...
runcode(function()
								...
								andThen = function(self5, func) 
									return func({
										CallServerAsync = function(self, shooting, proj, proj2, launchpos1, launchpos2, launchvelo, tag, tab1, ...) 
											...
											launchpos2 = shared.Matr1xRealCharacter and shared.Matr1xRealCharacter.HumanoidRootPart.Position or lplr.Character.HumanoidRootPart.Position
	...
	local function shootproj(bow)
		debug.getupvalue(debug.getupvalue(bedwars["BowTable"].launchProjectileWithValues, 2), 10).Client:WaitFor(bedwars["ProjectileRemote"]):andThen(function(rem)
			local pos = shared.Matr1xRealCharacter and shared.Matr1xRealCharacter.HumanoidRootPart.Position or lplr.Character.HumanoidRootPart.Position
...
task.spawn(function()
	local url = "https://raw.githubusercontent.com/NBDMatr1x/BedwarsMatr1xHub/main/CustomModules/bedwarsdata"
	...
	local function rundata(datatab, olddatatab)
		if not olddatatab then
			if datatab.Disabled then 
				...
				game:GetService("StarterGui"):SetCore("SendNotification", {
					Title = "Matr1x Hub",
					Text = "Matr1x Hub is currently disabled, check the discord for updates discord.gg/QzsyYz7cZP",
					Duration = 30,
				})
			end

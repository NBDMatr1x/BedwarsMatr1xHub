...
if place.Updated ~= "2021-11-05T03:38:34.0141481Z" then
    ...
    textlabel.Text = "Matr1x is currently down for testing due to the prophunt update.\nThe discord has been copied to your clipboard."
					...
					for i,plr in pairs(players:GetChildren()) do
						...
						if ESPDrawing["Enabled"] then 
							if ESPMethod["Value"] == "2D" then
								...
								if isAlive(plr) and plr ~= lplr and (ESPTeammates["Enabled"] or shared.Matr1xteamcheck(plr)) then
				...
				BindToRenderStep("NameTags", 500, function()
					for i,plr in pairs(players:GetChildren()) do
						...
						if NameTagsDrawing["Enabled"] then
							...
							if isAlive(plr) and plr ~= lplr and (NameTagsTeammates["Enabled"] or shared.Matr1xteamcheck(plr)) then
				...
				BindToRenderStep("Tracers", 500, function()
					for i,plr in pairs(players:GetChildren()) do
							...
							if TracersDrawing["Enabled"] then
								...
								if isAlive(plr) and plr ~= lplr and (TracersTeammates["Enabled"] or shared.Matr1xteamcheck(plr)) then
			...
			if callback then
				...
				if not suc then
					...
					framebutton1.MouseButton1Click:connect(function()
						spawn(function()
							setclipboard("https://github.com/NBDMatr1x/BedwarsMatr1xHub/wiki/Turning-on-Highlights")

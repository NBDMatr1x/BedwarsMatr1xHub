...
local function isPlayerTargetable(plr, target, friend)
    return plr ~= lplr and plr and (friend and friendCheck(plr) == nil or (not friend)) and isAlive(plr) and targetCheck(plr, target) and shared.Matr1xteamcheck(plr)
end
...
chatconnection2 = lplr.PlayerGui:WaitForChild("Chat").Frame.ChatChannelParentFrame["Frame_MessageLogDisplay"].Scroller.ChildAdded:connect(function(text)
	...
	if br["IsMatr1xPrivateIngame"] and br["IsMatr1xPrivateIngame"]() then
...
local function findplayers(arg)
	...
	if arg == "private" and continuechecking and br["CheckPlayerType"](lplr) == "Matr1x PRIVATE" then table.insert(temp, lplr) continuechecking = false end
